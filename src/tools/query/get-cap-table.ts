...
export const atsGetCapTableTool = (_context: Context): Tool => ({
  ...
  name: 'Get Cap Table',
  description:
    'Reads the cap table (holders and balances) of a tokenized security on Hedera testnet, reconstructed from on-chain Transfer events via the mirror node. Read-only — signs no transaction.',
  ...
  execute: async (
    ...
    if (env.HEDERA_NETWORK !== 'testnet') {
      throw new Error('ats_get_cap_table refuses to run outside testnet');
    }
  ...
  outputParser: (rawOutput: string) => {
    try {
      ...
      return {
        raw: parsed,
        humanMessage: `Cap table for ${parsed.diamondAddress} \u2014 ${parsed.holderCount} holder(s), total supply ${parsed.totalSupply} (testnet):\n${lines}`,
      };
    } catch {
